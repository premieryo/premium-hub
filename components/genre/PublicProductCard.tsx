...
export default function PublicProductCard({ product }: { product: Product }) {
  ...
  return <article className="flex h-full min-w-0 flex-col rounded-2xl border border-white/10 bg-slate-900 p-5">
    ...
    <div className="mt-5 flex flex-col gap-2">
      {product.url && product.priceTrackingEnabled && product.marketPrice ? <a href={product.url} target="_blank" rel="sponsored noopener noreferrer" className="min-h-11 rounded-xl border border-emerald-500/50 px-4 py-3 text-center text-sm font-bold text-emerald-300">PR｜Yahoo!ショッピングで商品を見る ↗</a> : null}
